...
const PIPELINES = [
  ...
  { id:'entrevistas-generales', nombre:'ENTREVISTAS GENERALES', icon:'🤝', visible:true,
    etapas:[],
    tabs:[{ id:'all', nombre:'Todos', sidebar:false, matchAll:true, etapas:[] }] },
...
function applyRolePermissions(role) {
  ...
  show('nav-users',            true); // todos pueden ver gestión de usuarios
}
...
async function openUsersModal() {
  ...
  if (createForm) createForm.style.display = currentUser?.role === 'developer' ? '' : 'none';
  await loadUsersList();
}
